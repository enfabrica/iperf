...
int iperf_sctp_listen(struct iperf_test *test) {
  ...
  if (test->bind_dev) {
    ...
    if (setsockopt(s, SOL_SOCKET, SO_BINDTODEVICE, test->bind_dev, IFNAMSIZ) <
        0)
...
int iperf_sctp_connect(struct iperf_test *test) {
  ...
  if (test->bind_address) {
    ...
    hints.ai_family = test->settings->domain;
    hints.ai_socktype = SOCK_STREAM;
    if ((gerror = getaddrinfo(test->bind_address, NULL, &hints, &local_res)) !=
        0) {
      i_errno = IESTREAMCONNECT;
      return -1;
    }
  ...
  if (test->bind_address) {
    ...
    if (bind(s, (struct sockaddr *)local_res->ai_addr, local_res->ai_addrlen) <
        0) {
      saved_errno = errno;
      close(s);
      freeaddrinfo(local_res);
