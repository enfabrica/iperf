...
int iperf_connect(struct iperf_test *test) {
  ...
  if (test->ctrl_sck < 0) {
    char *control = test->control_channel ?: test->server_hostname;

    // Create the control channel using an ephemeral port
    test->ctrl_sck =
        netdial(test->settings->domain, Ptcp, 0, 0, 0, control,
                test->server_port, test->settings->connect_timeout);
  }
...
int iperf_client_end(struct iperf_test *test) {
  ...
  SLIST_FOREACH(sp, &test->streams, streams) { close(sp->socket); }
  ...
  while (test->state != IPERF_DONE) {
    ...
    if (result < 0 && errno != EINTR) {
      ...
      goto cleanup_and_fail;
               ...
               rcv_timeout_us > 0) {
      // If nothing was received in non-reverse running state then probably
      // something got stack - either client, server or network, and test should
      // be terminated.
      iperf_time_now(&now);
...
cleanup_and_fail:
  ...
  if (test->json_output) {
    if (iperf_json_finish(test) < 0)
      return -1; // It is o.k. that error will be logged later outside the JSON
                 // output since its creation failed
  }
  iflush(test);
  return 0; // Return 0 and not -1 since all terminating function were done
            // here. Also prevents error message logging outside the already
            // closed JSON output.
}
