...
int iperf_tcp_recv(struct iperf_stream *sp) {
   ...
   * Have to handle case where pattern restarts to zero at any point in bfr
   * since back to back data blocks occur in TCP
   */
  ...
  if (sp->test->data_val == 1 && r > 1) {
    ...
    for (index = 0; index < r; index++) {
      if (index == 0) {
        ...
        // Pattern did not match.  Check for condition where this is start of
        // next bfr
        if (*ptrData == 0 && dblErr == 0) {

          // This appears to be start of next bfr, so do not fail
          // Set the double error flag so if next byte is also zero, we fail the
          // validation
          dblErr = 1;
          ptrData++;
          valCheck = 1;
          ...
          if (sp->data_error == 0)
            iperf_err(
                sp->test,
                "Validation Error- index = %d, expected = %d, actual = %d",
                index, valCheck, (int)*ptrData);
...
int iperf_tcp_accept(struct iperf_test *test) {
  ...
  if (!test->disable_cookie_check && strcmp(test->cookie, cookie) != 0) {
    if (Nwrite(s, (char *)&rbuf, sizeof(rbuf), Ptcp) < 0) {
      iperf_err(test,
                "failed to send access denied from busy server to new "
                "connecting client, errno = %d\n",
                errno);
    }
    close(s);
  }
...
int iperf_tcp_connect(struct iperf_test *test) {
  ...
  if (test->settings->flowlabel) {
    if (server_res->ai_addr->sa_family != AF_INET6) {
      ...
      opt = 1;
      if (setsockopt(s, IPPROTO_IPV6, IPV6_FLOWINFO_SEND, &opt, sizeof(opt)) <
          0) {
        saved_errno = errno;
        close(s);
        freeaddrinfo(server_res);
