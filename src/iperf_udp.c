    ...
    if (pcount >= sp->packet_count + 1) {
      ...
      if (sp->test->debug)
        fprintf(stderr,
                ...
                " but expected sequence %d on stream %d",
...
int iperf_udp_send(struct iperf_stream *sp) {
  ...
  // If data validation enabled, add pattern starting with ZERO after first 3
  // parameters
...
int iperf_udp_buffercheck(struct iperf_test *test, int s) {
  ...
  if (test->settings->blksize > sndbuf_actual) {
    char str[80];
    snprintf(str, sizeof(str), "Block size %d > sending socket buffer size %d",
...
int iperf_udp_accept(struct iperf_test *test) {
   ...
   */
  test->prot_listener =
      netannounce(test->settings->domain, Pudp, test->bind_address,
  ...
  test->max_fd =
      (test->max_fd < test->prot_listener) ? test->prot_listener : test->max_fd;
 ...
 */
int iperf_udp_init(struct iperf_test *test) { return 0; }
