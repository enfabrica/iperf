...
void usage() { fputs(usage_shortstr, stderr); }
...
void warning(const char *str) { fprintf(stderr, "warning: %s\n", str); }

/************** Getter routines for some fields inside iperf_test *************/

int iperf_get_verbose(struct iperf_test *ipt) { return ipt->verbose; }

int iperf_get_control_socket(struct iperf_test *ipt) { return ipt->ctrl_sck; }

int iperf_get_control_socket_mss(struct iperf_test *ipt) {
  return ipt->ctrl_sck_mss;
}

int iperf_get_test_omit(struct iperf_test *ipt) { return ipt->omit; }

int iperf_get_test_duration(struct iperf_test *ipt) { return ipt->duration; }
...
char iperf_get_test_role(struct iperf_test *ipt) { return ipt->role; }

int iperf_get_test_reverse(struct iperf_test *ipt) { return ipt->reverse; }

int iperf_get_test_blksize(struct iperf_test *ipt) {
  return ipt->settings->blksize;
}

FILE *iperf_get_test_outfile(struct iperf_test *ipt) { return ipt->outfile; }
...
int iperf_get_test_zerocopy(struct iperf_test *ipt) { return ipt->zerocopy; }
...
char *iperf_get_test_bind_dev(struct iperf_test *ipt) { return ipt->bind_dev; }

int iperf_get_test_udp_counters_64bit(struct iperf_test *ipt) {
  return ipt->udp_counters_64bit;
}

int iperf_get_test_one_off(struct iperf_test *ipt) { return ipt->one_off; }

int iperf_get_test_tos(struct iperf_test *ipt) { return ipt->settings->tos; }
...
char *iperf_get_iperf_version(void) { return (char *)iperf_version; }

int iperf_get_test_no_delay(struct iperf_test *ipt) { return ipt->no_delay; }
...
void iperf_set_test_omit(struct iperf_test *ipt, int omit) { ipt->omit = omit; }
...
int iperf_has_zerocopy(void) { return has_sendfile(); }
...
void iperf_set_test_bind_dev(struct iperf_test *ipt, char *bnd_dev) {
...
void iperf_on_new_stream(struct iperf_stream *sp) { connect_msg(sp); }
  ...
  if (test->json_output) {
    cJSON_AddItemToObject(
        ...
        iperf_json_printf(
            ...
            "%d  bytes: %d  blocks: %d  reverse: %d  tos: %d",
    ...
    if (test->verbose) {
      if (test->settings->bytes)
        iperf_printf(test, test_start_bytes, test->protocol->name,
                     test->num_streams, test->settings->blksize, test->omit,
                     test->settings->bytes, test->settings->tos);
      else if (test->settings->blocks)
        iperf_printf(test, test_start_blocks, test->protocol->name,
                     test->num_streams, test->settings->blksize, test->omit,
                     test->settings->blocks, test->settings->tos);
      else
        iperf_printf(test, test_start_time, test->protocol->name,
                     test->num_streams, test->settings->blksize, test->omit,
                     test->duration, test->settings->tos);
    }
  }
}
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv) {
  ...
  while ((flag = getopt_long(
              argc, argv,
              "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hxX:",
              longopts, NULL)) != -1) {
    switch (flag) {
    case 'p':
      portno = atoi(optarg);
    ...
    case 'f':
      ...
      break;
    case 'i':
      /* XXX: could potentially want separate stat collection and reporting
         intervals, but just set them to be the same for now */
  ...
  if (j == NULL) {
    ...
    if (test->settings->client_username && test->settings->client_password &&
      ...
      int rc = encode_auth_setting(
          test->settings->client_username, test->settings->client_password,
          test->settings->client_rsa_pubkey, &test->settings->authtoken);
    ...
    if (j_streams == NULL) {
      ...
      SLIST_FOREACH(sp, &test->streams, streams) {
        ...
        if (j_stream == NULL) {
          ...
          iperf_time_diff(&sp->result->start_time, &sp->result->start_time,
                          &temp_time);
    ...
    if (j_cpu_util_total == NULL || j_cpu_util_user == NULL ||
      ...
      if (j_streams == NULL) {
        ...
        if (test->role == 'c' && iperf_get_test_get_server_output(test)) {
          /* Look for JSON.  If we find it, grab the object so it doesn't get
           * deleted. */
          ...
          if (j_server_output != NULL) {
            ...
            /* No JSON, look for textual output.  Make a copy of the text for
             * later. */
...
void connect_msg(struct iperf_stream *sp) {
  ...
  if (getsockdomain(sp->socket) == AF_INET) {
    ...
    inet_ntop(AF_INET,
              (void *)&((struct sockaddr_in *)&sp->remote_addr)->sin_addr, ipr,
              sizeof(ipr));
    mapped_v4_to_regular_v4(ipr);
    ...
    inet_ntop(AF_INET6,
              (void *)&((struct sockaddr_in6 *)&sp->local_addr)->sin6_addr, ipl,
              sizeof(ipl));
    mapped_v4_to_regular_v4(ipl);
    inet_ntop(AF_INET6,
  ...
  if (sp->test->json_output)
    cJSON_AddItemToArray(
        ...
        iperf_json_printf("socket: %d  local_host: %s  local_port: %d  "
                          "remote_host: %s  remote_port: %d",
...
void protocol_free(struct protocol *proto) { free(proto); }
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
          if (test->sender_has_retransmits == 1 && stream_must_be_sender) {
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                  json_interval, "sum",
                  iperf_json_printf(
                      "start: %f  end: %f  seconds: %f  bytes: %d  "
                      "bits_per_second: %f  retransmits: %d  omitted: %b "
                      "sender: %b",
                      (double)start_time, (double)end_time,
                      (double)irp->interval_duration, (int64_t)bytes,
                      bandwidth * 8, (int64_t)retransmits, irp->omitted,
                      stream_must_be_sender)); /* XXX irp->omitted or
                                                  test->omitting? */
            else
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                  ...
                  iperf_json_printf(
                      "start: %f  end: %f  seconds: %f  bytes: %d  "
                      "bits_per_second: %f  omitted: %b sender: %b",
                      (double)start_time, (double)end_time,
                      (double)irp->interval_duration, (int64_t)bytes,
                      bandwidth * 8, test->omitting, stream_must_be_sender));
            else
          ...
          if (stream_must_be_sender) {
            if (test->json_output)
              cJSON_AddItemToObject(
                  json_interval, "sum",
                  iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: "
                                    "%d  bits_per_second: %f  packets: %d  "
                                    "omitted: %b sender: %b",
                                    (double)start_time, (double)end_time,
              ...
              iperf_printf(test, report_sum_bw_udp_sender_format, mbuf,
                           start_time, end_time, ubuf, nbuf, zbuf,
                           total_packets, test->omitting ? report_omitted : "");
          } else {
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                  ...
                  iperf_json_printf(
                      "start: %f  end: %f  seconds: %f  bytes: %d  "
                      "bits_per_second: %f  jitter_ms: %f  lost_packets: %d  "
                      "packets: %d  lost_percent: %f  omitted: %b sender: %b",
                      (double)start_time, (double)end_time,
                      (double)irp->interval_duration, (int64_t)bytes,
              ...
              iperf_printf(test, report_sum_bw_udp_format, mbuf, start_time,
                           end_time, ubuf, nbuf, avg_jitter * 1000.0,
                           lost_packets, total_packets, lost_percent,
                           test->omitting ? report_omitted : "");
          }
        }
      }
  ...
  for (current_mode = lower_mode; current_mode <= upper_mode; ++current_mode) {
      ...
      SLIST_FOREACH(sp, &test->streams, streams) {
        if (sp->sender == stream_must_be_sender) {
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            /*
             * Running total of the total number of packets.  Use the sender
             * packet count if we have it, otherwise use the receiver packet
             * count.
             */
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender_has_retransmits) {
              ...
              if (test->json_output)
                ...
                if (sp->data_error > 0)
                  iperf_printf(test, report_sum_data_error_packets,
                               sp->data_error);
              }
            } else {
              ...
              if (test->json_output)
                ...
                iperf_printf(test, report_bw_format, sp->socket, mbuf,
                             start_time, sender_time, ubuf, nbuf,
                             report_sender);
                if (sp->data_error > 0)
                  iperf_printf(test, report_sum_data_error_packets,
            ...
            if (test->json_output) {
              ...
              cJSON_AddItemToObject(
                  ...
                  iperf_json_printf(
                      "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  "
                      "bits_per_second: %f  jitter_ms: %f  lost_packets: %d  "
                      "packets: %d  lost_percent: %f  out_of_order: %d sender: "
                      "%b data_errors: %d",
                      (int64_t)sp->socket, (double)start_time,
                      (double)sender_time, (double)sender_time,
                      (int64_t)bytes_sent, bandwidth * 8,
                      ...
                      stream_must_be_sender, (int64_t)sp->data_error));
            } else {
              /*
              ...
              if ((sp->outoforder_packets - sp->omitted_outoforder_packets) > 0)
                iperf_printf(
                    test, report_sum_outoforder, mbuf, start_time, sender_time,
          ...
          if (sp->diskfile_fd >= 0) {
            if (fstat(sp->diskfile_fd, &sb) == 0) {
              /* In the odd case that it's a zero-sized file, say it was all
               * transferred. */
              int percent_sent = 100, percent_received = 100;
              if (sb.st_size > 0) {
                percent_sent =
                    (int)(((double)bytes_sent / (double)sb.st_size) * 100.0);
                percent_received =
                    (int)(((double)bytes_received / (double)sb.st_size) *
                          100.0);
              }
              ...
              if (test->json_output)
                cJSON_AddItemToObject(
                    ...
                    iperf_json_printf(
                        "sent: %d  received: %d  size: %d  percent_sent: %d  "
                        "percent_received: %d  filename: %s",
                        (int64_t)bytes_sent, (int64_t)bytes_received,
                        (int64_t)sb.st_size, (int64_t)percent_sent,
                        (int64_t)percent_received, test->diskfile_name));
                ...
                iperf_printf(test, report_diskfile, ubuf, sbuf,
                             percent_received, test->diskfile_name);
              }
            }
          }
          ...
          if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            ...
            if (test->json_output)
              cJSON_AddItemToObject(
                  json_summary_stream, "receiver",
                  iperf_json_printf(
                      "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  "
                      "bits_per_second: %f sender: %b data_errors: %d",
                      (int64_t)sp->socket, (double)start_time,
            ...
            if (!test->json_output) {
              if (receiver_packet_count - sp->omitted_packet_count > 0) {
                lost_percent =
                    100.0 * (sp->cnt_error - sp->omitted_cnt_error) /
                    (receiver_packet_count - sp->omitted_packet_count);
              } else {
                lost_percent = 0.0;
              }
    ...
    if (test->num_streams > 1 || test->json_output) {
      ...
      if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        if (test->sender_has_retransmits) {
          ...
          if (test->json_output)
            cJSON_AddItemToObject(
                test->json_end, "sum_sent",
                iperf_json_printf(
                    "start: %f  end: %f  seconds: %f  bytes: %d  "
                    "bits_per_second: %f  retransmits: %d sender: %b",
                    (double)start_time, (double)sender_time,
                    (double)sender_time, (int64_t)total_sent, bandwidth * 8,
                    (int64_t)total_retransmits, stream_must_be_sender));
            ...
            iperf_printf(test, report_sum_bw_retrans_format, mbuf, start_time,
                         sender_time, ubuf, nbuf, total_retransmits,
                         report_sender);
          }
        } else {
          ...
          if (test->json_output)
            cJSON_AddItemToObject(
                test->json_end, "sum_sent",
                iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  "
                                  "bits_per_second: %f sender: %b",
                                  (double)start_time, (double)sender_time,
        ...
        if (test->json_output)
          ...
          if (test->verbose)
            iperf_printf(test, report_receiver_not_available_summary_format,
                         "SUM");
        } else {
          iperf_printf(test, report_sum_bw_format, mbuf, start_time,
                       receiver_time, ubuf, nbuf, report_receiver);
        ...
        if (test->json_output)
          cJSON_AddItemToObject(
              ...
              iperf_json_printf(
                  "start: %f  end: %f  seconds: %f  bytes: %d  "
                  "bits_per_second: %f  jitter_ms: %f  lost_packets: %d  "
                  "packets: %d  lost_percent: %f sender: %b",
                  (double)start_time, (double)receiver_time,
                  (double)receiver_time, (int64_t)total_sent, bandwidth * 8,
          ...
          if (!(test->role == 's' && stream_must_be_sender)) {
            ...
            iperf_printf(test, report_sum_bw_udp_format, mbuf, start_time,
                         receiver_time, ubuf, nbuf, avg_jitter * 1000.0,
                         lost_packets, receiver_total_packets, lost_percent,
                         "receiver");
          }
        }
      }
    ...
    if (test->json_output && current_mode == upper_mode) {
      cJSON_AddItemToObject(
          ...
          iperf_json_printf(
              "host_total: %f  host_user: %f  host_system: %f  remote_total: "
              "%f  remote_user: %f  remote_system: %f",
      ...
      /* Print server output if we're on the client and it was
       * requested/provided */
      if (test->role == 'c' && iperf_get_test_get_server_output(test) &&
          !test->json_output) {
    ...
    if (test->sender_has_retransmits == 1 && sp->sender) {
      ...
      if (test->json_output)
        cJSON_AddItemToArray(
            ...
            iperf_json_printf(
                "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  "
                "bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  snd_wnd: "
                " %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: %b sender: %b",
                (int64_t)sp->socket, (double)st, (double)et,
                (double)irp->interval_duration, (int64_t)irp->bytes_transferred,
                bandwidth * 8, (int64_t)irp->interval_retrans,
                (int64_t)irp->snd_cwnd, (int64_t)irp->snd_wnd,
                (int64_t)irp->rtt, (int64_t)irp->rttvar, (int64_t)irp->pmtu,
                irp->omitted, sp->sender));
      else {
        unit_snprintf(cbuf, UNIT_LEN, irp->snd_cwnd, 'A');
        iperf_printf(test, report_bw_retrans_cwnd_format, sp->socket, mbuf, st,
                     et, ubuf, nbuf, irp->interval_retrans, cbuf,
      ...
      if (test->json_output)
        ...
        iperf_printf(test, report_bw_format, sp->socket, mbuf, st, et, ubuf,
                     nbuf, irp->omitted ? report_omitted : "");
    }
  } else {
    /* Interval, UDP. */
    if (sp->sender) {
      if (test->json_output)
        cJSON_AddItemToArray(
            ...
            iperf_json_printf(
                "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  "
                "bits_per_second: %f  packets: %d  omitted: %b sender: %b",
                (int64_t)sp->socket, (double)st, (double)et,
                (double)irp->interval_duration, (int64_t)irp->bytes_transferred,
                bandwidth * 8, (int64_t)irp->interval_packet_count,
                irp->omitted, sp->sender));
      else
        iperf_printf(test, report_bw_udp_sender_format, sp->socket, mbuf, st,
                     et, ubuf, nbuf, zbuf, irp->interval_packet_count,
                     irp->omitted ? report_omitted : "");
    } else {
      if (irp->interval_packet_count > 0) {
        lost_percent =
            100.0 * irp->interval_cnt_error / irp->interval_packet_count;
      } else {
        lost_percent = 0.0;
      }
      if (test->json_output)
        cJSON_AddItemToArray(
            ...
            iperf_json_printf(
                ...
                (int64_t)irp->interval_packet_count, (double)lost_percent,
                irp->omitted, sp->sender));
      else
        iperf_printf(test, report_bw_udp_format, sp->socket, mbuf, st, et, ubuf,
                     nbuf, irp->jitter * 1000.0, irp->interval_cnt_error,
...
struct iperf_stream *iperf_new_stream(struct iperf_test *test, int s,
  ...
  if (test->zc_api) {
    sp->buffer =
        (char *)mmap(NULL, test->settings->blksize, PROT_READ | PROT_WRITE,
...
int iperf_init_stream(struct iperf_stream *sp, struct iperf_test *test) {
...
#ifdef IPV6_TCLASS
      if (setsockopt(sp->socket, IPPROTO_IPV6, IPV6_TCLASS, &opt, sizeof(opt)) <
          0) {
        i_errno = IESETCOS;
        return -1;
      }
  ...
  if (iperf_get_test_protocol_id(test) == Pudp &&
    ...
    opt = IP_PMTUDISC_DO;
    if (setsockopt(sp->socket, IPPROTO_IP, IP_MTU_DISCOVER, &opt, sizeof(opt)) <
        0) {
    ...
    opt = 1;
    if (setsockopt(sp->socket, IPPROTO_IP, IP_DONTFRAGMENT, &opt, sizeof(opt)) <
        0) {
      i_errno = IESETDONTFRAGMENT;
      return -1;
    }
...
int iflush(struct iperf_test *test) { return fflush(test->outfile); }
