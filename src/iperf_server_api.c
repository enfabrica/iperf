...
retry:
  if ((test->listener =
           netannounce(test->settings->domain, Ptcp, test->bind_address,
...
int iperf_run_server(struct iperf_test *test) {
  ...
  while (test->state != IPERF_DONE) {
    ...
    if (result < 0 && errno != EINTR) {
      ...
      // then probably something got stack either at the client, server or
      // network, and Test should be forced to end.
      iperf_time_now(&now);
      t_usecs = 0;
      if (iperf_time_diff(&now, &last_receive_time, &diff_time) == 0) {
        ...
        if (test->state == IPERF_START) {
          if (test->settings->idle_timeout > 0 &&
            ...
            if (test->debug)
              printf("Server restart (#%d) in idle state as no connection "
                     "request was received for %d sec\n",
          ...
          if (iperf_get_verbose(test))
            iperf_err(test,
                      "Server restart (#%d) during active test due to idle "
                      "data for receiving data",
          ...
          if (!is_closed(s)) {
            ...
            if (test->protocol->id == Ptcp) {
              ...
              }
              {
                ...
                // Set actual used congestion alg, or set to unknown if could
                // not get it
                if (rc < 0)
