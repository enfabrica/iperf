...
void iperf_err(struct iperf_test *test, const char *format, ...) {
  ...
  if (test != NULL && test->timestamps) {
    time(&now);
    ltm = localtime(&now);
    strftime(iperf_timestrerr, sizeof(iperf_timestrerr), test->timestamp_format,
             ltm);
    ct = iperf_timestrerr;
  }
...
char *iperf_strerror(int int_errno) {
  ...
  case IESETCLIENTAUTH:
    snprintf(errstr, len,
             "you must specify a username, password, and path to a valid RSA "
             "public key");
    break;
  case IESETSERVERAUTH:
    snprintf(errstr, len,
             "you must specify a path to a valid RSA private key and a user "
             "credential file");
    break;
  case IEBADFORMAT:
    snprintf(errstr, len,
  ...
  case IEIDLETIMEOUT:
    snprintf(
        errstr, len,
        "idle timeout parameter is not positive or larget then allowed limit");
    break;
  case IENOMSG:
